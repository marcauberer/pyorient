...
ScriptFunction = namedtuple('Method', ['definition', 'signature', 'body', 'sha1'])
...
class Scripts(object):
    ...
    def script_body(self, name, args = None, namespace = None):
        ...
        if isinstance(args, dict):
            param_defaults = OrderedDict([val + (None,) if len(val) == 1 else val for val in params])
            ...
            args = { params[i][0]: args[i] if i < len(args) else literal_eval(params[i][1])
                        for i in range(0, max(len(params), len(args))) if i < len(args) or len(params[i]) == 2}
        else:
            if params:
                if args:
        ...
        for i, s in enumerate(split_body):
            if i % 4 == 0:
                for k, v in replacements.items():
                    split_body[i] = re.sub(r'\b{}\b'.format(k), v, split_body[i])
...
class ParamDefault(tuple):
    def __new__(self, pair):
        ...
        return tuple.__new__(self, (pair[0].strip(),) + ((pair[1],) if l > 1 else tuple()))
