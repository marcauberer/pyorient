from .operators import Operator, IdentityOperand, Operand, ArithmeticOperation, LogicalConnective
...
class Query(object):
    ...
    def __iter__(self):
        ...
        with TempParams(params, skip='#-1:-1', limit=1):
            ...
            if len(prop_names) > 1:
                ...
                selectuple = namedtuple(prop_prefix + '_props', [Query.sanitise_prop_name(name) for name in prop_names])
            wheres = self.build_wheres(params)

            g = self._graph
            while True:
                current_skip = params['skip']
                where = u'WHERE {0}'.format(u' and '.join([self.rid_lower(current_skip)] + wheres))
                ...
                if response:
                    ...
                    if prop_names:
                        ...
                        if next_skip:
                            ...
                            if len(prop_names) > 1:
                                yield selectuple(*tuple(self.parse_record_prop(response.oRecordData.get(name))
                                                        for name in prop_names))
                            else:
                                yield self.parse_record_prop(response.oRecordData[prop_names[0]])
                        else:
        ...
        if response:
            ...
            if list_query:
                if prop_names:
                    if len(prop_names) > 1:
                        return [
                            selectuple(*tuple(self.parse_record_prop(record.oRecordData.get(name))
                                              for name in prop_names))
    ...
    def one(self, reify=False):
        with TempParams(self._params, limit=2):
            ...
            if num_responses > 1:
                raise MultipleResultsFound('Expecting one result for query; got more.')
    ...
    def filter_string(cls, expression_root):
        ...
        if isinstance(left, IdentityOperand):
            if isinstance(left, Property):
                ...
                raise ValueError('Operator {} not supported as a filter'.format(op))

            if op is Operator.Equal:
                return u'{0} = {1}'.format(left_str, ArgConverter.convert_to(ArgConverter.Vertex, right, cls))
            elif op is Operator.GreaterEqual:
                return u'{0} >= {1}'.format(left_str, ArgConverter.convert_to(ArgConverter.Value, right, cls))
                ...
                return u'{0} BETWEEN {1} and {2}'.format(left_str, PropertyEncoder.encode_value(right), far_right)
            elif op is Operator.Contains:
                if isinstance(right, LogicalConnective):
                    return u'{0} contains({1})'.format(left_str, cls.filter_string(right))
                else:
                    return u'{} in {}'.format(PropertyEncoder.encode_value(right), left_str)
                ...
                return u'{0} matches {1}'.format(left_str, PropertyEncoder.encode_value(right))
            elif op is Operator.StartsWith:
                return u'{0} like {1}'.format(left_str, PropertyEncoder.encode_value(right + '%'))
            elif op is Operator.InstanceOf:
                return u'{0} instanceof {1}'.format(left_str, repr(right.registry_name))
            else:
                raise AssertionError('Unhandled Operator type: {}'.format(op))
        else:
            return u'{0} {1} {2}'.format(cls.filter_string(left), 'and' if op is Operator.And else 'or',
    ...
    def arithmetic_string(cls, operation_root):
        if isinstance(operation_root, ArithmeticOperation):
            ...
            if op is Operator.Add:
                exp = '{} + {}'.format(cls.arithmetic_string(left), cls.arithmetic_string(right))
    ...
    def build_props(self, params, prop_names=None, for_iterator=False):
        let = params.get('let')
        if let:
            lets = ['LET {}'.format(','.join('${} = {}'.format(PropertyEncoder.encode_name(k),
                    u'({})'.format(v) if isinstance(v, Query) else self.build_what(v)) for k,v in let.items()))]
        else:
            lets = []
    ...
    def build_wheres(self, params):
        ...
        kw_where = [u' and '.join(u'{0}={1}'.format(PropertyEncoder.encode_name(k), PropertyEncoder.encode_value(v))
    ...
    def build_optional_clauses(self, params, skip):
        ...
        if group_by:
            group_clause = 'GROUP BY {}'.format(','.join([by.context_name() for by in group_by]))
            optional_clauses.append(group_clause)

        order_by = params.get('order_by')
        if order_by:
            order_clause = 'ORDER BY {0} {1}'.format(','.join([by.context_name() for by in order_by[0]]),
                                                     'DESC' if order_by[1] else 'ASC')
            optional_clauses.append(order_clause)

        unwind = params.get('unwind')
        if unwind:
           unwind_clause = 'UNWIND {}'.format(unwind.context_name() if isinstance(unwind, Property) else unwind)
    ...
    WhatFunctions = {
        ...
        What.Coalesce: WhatFunction(None, 'coalesce({})', (ArgConverter.Field,)),
        What.If: WhatFunction(3, 'if({})', (ArgConverter.Boolean, ArgConverter.Value, ArgConverter.Value)),
        What.IfNull: WhatFunction(2, 'ifnull({})', (ArgConverter.Field, ArgConverter.Value)),
        ...
        What.Percentile: WhatFunction(None, 'percentile({})', (ArgConverter.Field,)),
        ...
        What.Date: WhatFunction(3, 'date({})', (ArgConverter.String, ArgConverter.String, ArgConverter.String)),
        What.SysDate: WhatFunction(2, 'sysdate({})', (ArgConverter.String, ArgConverter.String)),
        What.Format: WhatFunction(None, 'format({})', (ArgConverter.Format, ArgConverter.Field)),
        What.Dijkstra: WhatFunction(4, 'dijkstra({})', (ArgConverter.Vertex, ArgConverter.Vertex,
                                                          ArgConverter.Label, ArgConverter.Value)),
        What.ShortestPath: WhatFunction(4, 'shortestPath({})', (ArgConverter.Vertex, ArgConverter.Vertex,
                                                                  ArgConverter.Value, ArgConverter.Label)),
        What.Distance: WhatFunction(4, 'distance({})', (ArgConverter.Field, ArgConverter.Field,ArgConverter.Value,
                                                        ArgConverter.Value)),
        What.Distinct: WhatFunction(1, 'distinct({})', (ArgConverter.Field,)),
        What.UnionAll: WhatFunction(None, 'unionall({})', (ArgConverter.Field,)),
        What.Intersect: WhatFunction(None, 'intersect({})', (ArgConverter.Field,)),
        What.Difference: WhatFunction(None, 'difference({})', (ArgConverter.Field,)),
        What.SymmetricDifference: WhatFunction(None, 'symmetricDifference({})', (ArgConverter.Field,)),
        ...
        What.Map: WhatFunction(2, 'map({})', (ArgConverter.Field, ArgConverter.Field)),
        What.TraversedElement: WhatFunction(2, 'traversedElement({})', (ArgConverter.Value, ArgConverter.Value)),
        What.TraversedEdge: WhatFunction(2, 'traversedEdge({})', (ArgConverter.Value, ArgConverter.Value)),
        What.TraversedVertex: WhatFunction(2, 'traversedVertex({})', (ArgConverter.Value, ArgConverter.Value)),
    ...
    def append_what_function(cls, chain, func_key, func_args):
        ...
        if max_args > 0 or max_args is None:
            chain.append(what_function.fmt.format(','.join(cls.what_args(what_function.expected, func_args[1]))))
        else:
    ...
    def build_what(cls, what, prop_names=None):
        ...
        if isinstance(what, FunctionWhat):
            ...
            if prop_names is not None:
                ...
                if counted:
                    prop_names.append(cls.parse_prop_name(what_function.fmt, name_override))

            return '{}{}'.format(
                what_function.fmt.format(','.join(cls.what_args(what_function.expected, what.chain[0][1]))), as_str)
            ...
            if prop_names is not None:
                prop_names.append(cls.parse_prop_name(chain[0], name_override))
    ...
    def what_args(cls, expected, args):
        if args:
            return [ArgConverter.convert_to(conversion, arg, cls)
                    for arg, conversion in zip_longest(args, expected, fillvalue=expected[-1]) if arg is not None]
        else:
            return []
    ...
    def build_select(self, props, optional_clauses):
        ...
        if props:
            return u'SELECT {} FROM {} {}'.format(','.join(props), src, optional_string)
        else:
