...
class Batch(object):
        ...
        if isinstance(key, slice):
            if key.step:
                ...
                if Batch.clean_name:
                    ...
                    raise ValueError('Variable name \'{}\' contains invalid character(s).'.format(key.stop))
    ...
    def return_string(variables):
        ...
        if isinstance(variables, (list, tuple)):
            return '[' + ','.join('${}'.format(cleaned(var)) for var in variables) + ']'
        elif isinstance(variables, dict):
            return '{' + ','.join('{}:${}'.format(repr(k), cleaned(v)) for k, v in variables.items()) + '}'
        else:
...
class BatchVertexVector(VertexVector):
    ...
    def __gt__(self, target):
        ...
        if hasattr(target, '_id'):
            return self.edge_broker.create_command(self.origin, target, **self.kwargs)
        return self
    ...
    def __lt__(self, origin):
        ...
        if hasattr(origin, '_id'):
            return self.edge_broker.create_command(origin, self.origin, **self.kwargs)
        return self
