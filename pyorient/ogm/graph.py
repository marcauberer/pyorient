...
class Graph(object):
    ...
    def open(self, db_name, storage, user=None, cred=None):
        ...
        if not self.client.db_exists(db_name, storage):
            self.client.db_create(db_name, pyorient.DB_TYPE_GRAPH, storage)
        ...
        self.server_version = ServerVersion(self.client.version.major, self.client.version.minor,
                                            self.client.version.build)
        return cluster_map
    ...
    def build_mapping(self, vertex, edge, auto_plural=False):
        ...
        schema = self.client.command('SELECT FROM (SELECT expand(classes) FROM metadata:schema) WHERE name NOT IN '
                                     '[\'ORole\', \'ORestricted\', \'OTriggered\', \'ORIDs\', \'OUser\', \'OIdentity\','
                                     ' \'OSchedule\', \'OFunction\']')
    ...
    def populate(self, load_path, fmt=None, preserve_cluster_ids=None, delete_rid_mapping=None, merge=None,
                 migrate_links=None, rebuild_indexes=None):
        ...
        def_import = 'def dbImport = new com.orientechnologies.orient.core.db.tool.ODatabaseImport(db, "{}", listener);'
        ...
        config = self.config
        import_groovy = self._GROOVY_GET_DB.format(config.host, config.port, config.db_name, config.user,
                                                   config.cred) + self._GROOVY_TRY.format(self._GROOVY_NULL_LISTENER,
                                                                                          import_commands)

        self.client.gremlin(import_groovy)

    def export(self, save_path, exclude_all=None, include_classes=None, exclude_classes=None, include_clusters=None,
               exclude_clusters=None, include_info=None, cluster_definitions=None, schema=None, security=None,
               records=None, index_defs=None, manual_indexes=None, compression_level=None, buffer_size=None):
        ...
        def_export = 'def export = new com.orientechnologies.orient.core.db.tool.ODatabaseExport(db, "{}", listener);'
        ...
        config = self.config
        export_groovy = self._GROOVY_GET_DB.format(config.host, config.port, config.db_name, config.user,
                                                   config.cred) + self._GROOVY_TRY.format(self._GROOVY_NULL_LISTENER,
                                                                                          export_commands)
    ...
    def create_class(self, cls):
        ...
        if not bases:
            raise TypeError('Unexpected base class(es) in Graph.create_class - try the declarative bases')
        ...
        #        .format(cls_name)):
        try:
            self.client.command('CREATE CLASS {0} EXTENDS {1}'.format(cls_name, extends))
        except pyorient.PyOrientSchemaException:
            # Class already exists
            pass

        props = sorted([(k, v) for k, v in cls.__dict__.items() if isinstance(v, Property)],
        ...
        for prop_name, prop_value in props:
            ...
            if isinstance(prop_value, LinkedClassProperty) and prop_value.linked_to is not None:
                ...
                if type_linked_to.__dict__.get('registry_name', None):
                    ...
                    if link_bases and isinstance(prop_value, LinkedProperty) and link_bases[0] is Property:
                        linked_to = type_linked_to.__name__

            try:
                self.client.command('CREATE PROPERTY {0} {1} {2}'.format(class_prop, type(prop_value).__name__,
                                                                         linked_to or ''))
            ...
            if prop_value.default is not None:
                if self.server_version >= (2, 1, 0):
                    self.client.command('ALTER PROPERTY {0} DEFAULT {1}'.format(class_prop,
                                    PropertyEncoder.encode_value(prop_value.default)))

            self.client.command('ALTER PROPERTY {0} NOTNULL {1}'.format(class_prop,
                                                                        str(not prop_value.nullable).lower()))

            self.client.command('ALTER PROPERTY {} MANDATORY {}'.format(class_prop,
                                                                        str(prop_value.mandatory).lower()))

            self.client.command('ALTER PROPERTY {} READONLY {}'.format(class_prop,
                                                                       str(prop_value.readonly).lower()))

            # TODO Add support for composite indexes
            if prop_value.indexed:
                try:
                    self.client.command('CREATE INDEX {0} {1}'.format(class_prop, 'UNIQUE' if prop_value.unique else 'NOTUNIQUE'))
    ...
    def drop_class(self, cls, ignore_instances=False):
        ...
        *will* still throw if subclassed.
        """
        if ignore_instances:
            self.client.command('DROP CLASS {} UNSAFE'.format(cls.registry_name))
        else:
            self.client.command('DROP CLASS {}'.format(cls.registry_name))
    ...
    def create_vertex_command(self, vertex_cls, **kwargs):
        ...
        if kwargs:
            ...
            set_clause = u' SET {}'.format(u','.join(u'{}={}'.format(
                PropertyEncoder.encode_name(k), PropertyEncoder.encode_value(v)) for k, v in db_props.items()))
        else:
            set_clause = u''

        return VertexCommand(u'CREATE VERTEX {}{}'.format(class_name, set_clause))
    ...
    def delete_vertex_command(self, vertex, where=None, limit=None, batch=None):
        ...
        if where is not None:
            ...
            if isinstance(where, dict):
                where_clause = u' and '.join(u'{0}={1}'.format(
                    PropertyEncoder.encode_name(k), PropertyEncoder.encode_value(v)) for k, v in where.items())
            else:
    ...
    def create_edge_command(self, edge_cls, from_vertex, to_vertex, **kwargs):
        ...
        return CreateEdgeCommand(u'CREATE EDGE {} FROM {} TO {}{}'.format(class_name, from_vertex._id, to_vertex._id,
                                                                          set_clause))
    ...
    def create_function(self, name, code, parameters=None, idempotent=False, language='javascript'):
        ...
        self.client.command(u'CREATE FUNCTION {} \'{}\' {} IDEMPOTENT {} LANGUAGE {}'.format(
    ...
    def save_element(self, element_class, props, elem_id):
        ...
        if isinstance(element_class, str):
            ...
            if not element_class:
                raise KeyError('Class \'{}\' not registered with graph.'.format(name))
    ...
    def vertex_from_record(self, record, vertex_cls=None):
        ...
        return vertex_cls.from_graph(self, record._rid, self.props_from_db[vertex_cls](props)) if vertex_cls \
    ...
    def edge_from_record(self, record, edge_cls=None):
        ...
        return edge_cls.from_graph(self, record._rid, in_hash, out_hash, self.props_from_db[edge_cls](props))\
            if edge_cls else Edge.from_graph(self, record._rid, in_hash, out_hash, props)
    ...
    def element_from_record(self, record):
        ...
        try:
            if isinstance(record_data['in'], pyorient.OrientRecordLink) and \
                    isinstance(record_data['out'], pyorient.OrientRecordLink):
    ...
    def guard_reserved_words(word, cls):
        ...
        if word in reserved_words:
            ...
            raise ReservedWordError("'{0}' as a property name will render class '{1}' unusable.".format(word,
                                                cls.registry_name))
    ...
    def props_to_db(element_class, props, strict):
        ...
        for k, v in props.items():
            ...
            if hasattr(element_class, k):
                ...
                raise AttributeError('Class {} has no property {}'.format(element_class, k))
    ...
    def init_broker_for_class(self, cls):
        ...
        if broker_name and not getattr(cls, 'no_graph_broker', False):
            if hasattr(self, broker_name):
                raise RuntimeError('Attempt to use a broker name reserved by Graph. Could use a different name, or set '
                                   'the \'no_graph_broker\' attribute to True for this element class.')
    ...
    def toposort_classes(classes):
        ...
        def get_class_topolist(class_name, name_to_class, processed_classes, current_trace):
            ...
            if class_name in current_trace:
                raise AssertionError('Encountered self-reference in dependency chain of {}'.format(class_name))
            ...
            for dependency in dependencies:
                class_list.extend(get_class_topolist(dependency, name_to_class, processed_classes, current_trace))
