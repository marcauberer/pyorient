...
class DeclarativeMeta(type):
    ...
    def __init__(cls, class_name, bases, attrs):
        if not hasattr(cls, 'registry'):
            ...
            decl_bases = set(
                base.decl_root for base in bases if hasattr(base, 'decl_root') and base is not base.decl_root
            )
            if len(decl_bases) > 1:
                raise TypeError('When multiply-inheriting graph elements, they must share the same declarative base'
                                'class. Note: Each call to declarative_*() returns a new base class.')

            if cls.decl_type is DeclarativeType.Vertex:
                cls.registry_name = attrs.get('element_type', cls.__name__.lower())
    ...
    def __setattr__(self, name, value):
        if isinstance(value, Property):
            if value.context:
                raise ValueError('Attempt to add a single Property to multiple classes.')
            value.context = self
...
def declarative_node(name='Node', cls=Vertex, metaclass=DeclarativeMeta, **kwargs):
    """Create base class for graph nodes/vertexes"""
    return declarative_base(DeclarativeType.Vertex, name, cls, metaclass, **kwargs)


def declarative_relationship(name='Relationship', cls=Edge, metaclass=DeclarativeMeta, **kwargs):
    ...
    """
    return declarative_base(DeclarativeType.Edge, name, cls, metaclass, **kwargs)
