...
from ..constants import DB_OPEN_OP, DB_TYPE_DOCUMENT, DB_COUNT_RECORDS_OP, FIELD_BYTE, FIELD_INT, \
    ...
    VERSION, DB_TYPES, DB_CLOSE_OP, DB_EXIST_OP, STORAGE_TYPE_PLOCAL, DB_CREATE_OP, \
...
class DbOpenMessage(BaseMessage):
    ...
    def set_db_type(self, db_type):
        if db_type in DB_TYPES:
            ...
            raise PyOrientBadMethodCallException(db_type + ' is not a valid database type', [])
        return self
...
class DbExistsMessage(BaseMessage):
    ...
    def set_storage_type(self, storage_type):
        if storage_type in STORAGE_TYPES:
            ...
            raise PyOrientBadMethodCallException(storage_type + ' is not a valid storage type', [])
        return self
...
class DbCreateMessage(BaseMessage):
    ...
    def prepare(self, params=None):
        ...
        self._append((FIELD_STRINGS, [self._db_name, self._db_type, self._storage_type]))
...
class DbDropMessage(BaseMessage):
    def __init__(self, _orient_socket):
        super(DbDropMessage, self).__init__(_orient_socket)
