...
from ..constants import QUERY_SYNC, QUERY_ASYNC, QUERY_CMD, QUERY_SCRIPT, QUERY_GREMLIN, FIELD_BYTE, FIELD_STRING,\
    FIELD_INT, FIELD_CHAR, FIELD_BOOLEAN, FIELD_SHORT, FIELD_LONG, COMMAND_OP, TX_COMMIT_OP, QUERY_TYPES
from ..utils import need_db_opened, need_connected
...
class CommandMessage(BaseMessage):
    ...
    def set_callback(self, func):
        """
        Setter method for the callback function
        """
...
class _TXCommitMessage(BaseMessage):
    ...
    def fetch_response(self):
        ...
        for x in range(0, items):
            ...
            operation = self._pre_operation_records[str(result['created'][-1]['client_c_pos'])]
        ...
        for x in range(0, items):
            ...
            try:
                operation = self._pre_operation_records[str(result['updated'][-1]['updated_c_pos'])]
                record = getattr(operation, "_record_content")
                rid = "#" + str(result['updated'][-1]['updated_c_id']) + \
                      ":" + str(result['updated'][-1]['updated_c_pos'])
                record.update(__version=result['updated'][-1]['new_version'], __rid=rid)
    ...
    def attach(self, operation):
        ...
        if isinstance(operation, RecordUpdateMessage):
            ...
            self._operation_stack[-1] = self._operation_stack[-1] + \
                ((FIELD_BOOLEAN, bool(getattr(operation, "_update_content"))),)
            self._pre_operation_records[str(getattr(operation, "_cluster_position"))] = operation
            ...
            self._pre_operation_records[str(self._temp_cluster_position_seq)] = operation
            self._temp_cluster_position_seq -= 1
        else:
            raise PyOrientBadMethodCallException("Wrong command type " + operation.__class__.__name__, [])
    ...
    def get_transaction_id(self):
        if self._tx_id < 0:
            ...
            # not a real UUID
            if _ms_time & 0x80000000:
                self._tx_id = int((_ms_time - 0x80000000) & 0xFFFFFFFF)
            else:
                self._tx_id = int(_ms_time & 0xFFFFFFFF)
