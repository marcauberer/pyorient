...
def binary_db_connect():
    ...
    cluster_info = DB.db_open(db_name, "admin", "admin", pyorient.DB_TYPE_GRAPH, "")

    return DB


def skip_binary_if_pyorient_native_not_installed(func):
    from pyorient.serializations import binary_can_be_used

    test_instance = isinstance(func, type)
    if not test_instance:
        if not binary_can_be_used:
...
class SerializationTestCase(unittest.TestCase):
    ...
    #         print(e)
    #     finally:
    #         db = DB.db_create(db_name, pyorient.DB_TYPE_GRAPH, pyorient.STORAGE_TYPE_MEMORY)
    #         pass
    #
