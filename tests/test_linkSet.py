...
class LinkSetTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client.db_open(db_name, "admin", "admin", pyorient.DB_TYPE_GRAPH, "")
        ...
        self.client.command("CREATE LINK link TYPE LINKSET FROM links.siteId TO sites.id INVERSE")
    ...
    def testEmbed(self):
        ...
        self._links_cluster_id = self.client.command("create class test_embed extends V")[0]

        self.client.command("create vertex test_embed set embMap = {'en': 'english','it':'italian', 'ru': 'russian'}")
    ...
    def testEmbedNum(self):
        ...
        self.client.command("create vertex test_embed set embMap = {'en': 1,'it':2, 'ru':3}")
    ...
    def testEmbeddedMapsInList(self):
        class_id1 = self.client.command("CREATE VERTEX V SET mapInList = [ {'one': 2, 'three': 4 } ]")[0].oRecordData
    ...
    def testEmptyEmbeddedMapsInList(self):
        # self.skipTest('Bug')
        print(1)
        class_id1 = self.client.command("CREATE VERTEX V CONTENT {'a': [ { 'b': [], 'c': [] } ] }")[0].oRecordData
